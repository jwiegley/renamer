...
instance (MonadParallel m) => MonadParallel (ReaderT e m) where
    parallelInterleaved xs = do
        e <- ask
        lift $ parallelInterleaved (Prelude.map (`runReaderT` e) xs)
    stopGlobalPool = lift stopGlobalPool

instance (MonadParallel m) => MonadParallel (StateT s m) where
    parallelInterleaved xs = do
        s <- get
        lift $ parallelInterleaved (Prelude.map (`evalStateT` s) xs)
    stopGlobalPool = lift stopGlobalPool
...
makeReadonly f = do
    p <- getPermissions f
    setPermissions f (p{Dir.writable = False})
